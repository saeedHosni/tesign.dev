...
export default function ProjectBanner() {
  ...
  const handleSubmit = async () => {
    ...
    const payload = {
      source: 'banner',
      ...(isEmail ? { email: value.trim() } : { phone: value.trim() }),
    };
    ...
    try {
      ...
      setApiError(err.message || 'خطا در ارسال اطلاعات. لطفاً دوباره تلاش کنید.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <section id="project" className="py-24 bg-bg-base">
      <div className="w-full max-w-[1200px] mx-auto px-6">
        <div className="reveal bg-bg-card border border-border-default rounded-xl p-[72px_60px] max-sm:p-12 max-sm:px-7 relative overflow-hidden text-center">
          ...
          <SectionLabel className="justify-center">ثبت پروژه</SectionLabel>
          ...
          <p className="text-[1rem] text-text-secondary max-w-[520px] mx-auto mb-10 leading-[1.8] relative z-[1]">
            جزئیات پروژه خود را ثبت کنید. تیم ما ظرف ۲۴ ساعت با شما تماس می‌گیرد و مشاوره رایگان ارائه می‌دهد.
          </p>
          ...
          {/* API error message */}
          {apiError && (
            <p className="mt-3 text-[0.82rem] text-accent-orange relative z-[1]">
              {apiError}
            </p>
          )}

          {/* Features */}
          <div className="flex justify-center gap-8 flex-wrap mt-10 relative z-[1]">
            {BANNER_FEATURES.map((f) => (
              <div key={f.label} className="flex items-center gap-2 text-[0.82rem] text-text-muted">
                <div className="w-[22px] h-[22px] bg-[rgba(245,197,24,0.1)] rounded-full grid place-items-center text-[0.65rem] text-accent-yellow">
                  {f.icon}
                </div>
                {f.label}
              </div>
